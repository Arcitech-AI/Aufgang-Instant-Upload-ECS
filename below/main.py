...
MAX_INSTANT_UPLOAD_CHARACTERS = 200_000
...
class StandaloneOpenSearchService:
    ...
    _INDEX_SETTINGS = {
        "analysis": {
            "filter": {
                ...
                "content_possessive_stemmer": {
                    "type": "stemmer",
                    "language": "possessive_english",
                },
            },
            "analyzer": {
                # Index-time: possessive stemmer → lowercase → stop → stemmer → ascii
                "content_analyzer": {
                    "type": "custom",
                    "tokenizer": "standard",
                    "filter": [
                ...
                # Search-time: same but skip possessive stemmer so phrases match cleanly
    ...
    _INDEX_MAPPINGS = {
        "properties": {
            ...
            "s3_key":     {"type": "keyword"},
            "file_size":  {"type": "long"},
            # ── Chunk payload (primary search field) ───────────────────────
            "content": {
                "type": "text",
                "analyzer": "content_analyzer",
                "search_analyzer": "content_search_analyzer",
                # .keyword sub-field for exact/aggregation queries
                "fields": {"keyword": {"type": "keyword", "ignore_above": 512}},
            },
            "page_number":  {"type": "integer"},
            "chunk_index":  {"type": "integer"},
            # ── Dense vector for k-NN / semantic search ────────────────────
            "embedding": {
                "type": "knn_vector",
            ...
            # ── Access control ─────────────────────────────────────────────
            "is_access_restricted": {"type": "boolean"},
            "access_permissions":   {"type": "object", "enabled": True},
            # ── Timestamps ─────────────────────────────────────────────────
    ...
    def _ensure_index(self) -> None:
        """
        Create the main index with custom analyzer and explicit mappings if it
        does not already exist.  Existing indices are left untouched — analyzer
        changes on live indices require a full reindex.
        """
        if not self.index_name:
            logger.warning("_ensure_index: OPENSEARCH_INDEX_NAME is not set — skipping")
            return
        try:
            if self.client.indices.exists(index=self.index_name):
                logger.debug(
                    "_ensure_index: index '%s' already exists — skipping creation",
                    self.index_name,
                )
                return
        except Exception as e:
            logger.warning("_ensure_index: could not check index '%s': %s", self.index_name, e)
            return
        ...
        try:
            ...
            logger.info(
                "_ensure_index: created index '%s' with content_analyzer "
                "(English stemming, stop-words, ASCII folding, k-NN vector)",
                self.index_name,
            )
        except Exception as e:
            # Non-fatal — the cluster may have auto-created the index already
            # or a race condition occurred between the exists() check and create().
            logger.warning("_ensure_index: could not create index '%s': %s", self.index_name, e)
...
class StandaloneVectorizationService:
    ...
    async def _extract_pdf_text(
        ...
        # Docling enrichment: run only on weak pages to avoid full-PDF ML overhead
        if VECTOR_PDF_DOCLING and page_texts is not None:
            ...
            if not weak_pns:
                ...
                dl = await asyncio.to_thread(_extract_docling_for_weak_pages, await _bytes(), weak_pns)
                if dl:
                    merged = []
                    for text, pn in page_texts:
            ...
            for model in ("gpt-4o", "gpt-4o-mini"):
                try:
                    ...
                    if "invalid_api_key" in err or "incorrect api key" in err or "401" in err:
                        logger.error(
                            "_caption_image: OPENAI KEY INVALID OR EXPIRED — "
                            "check OPENAI_API_KEY env var | model=%s | error=%s",
                            model, e,
                        )
                        return None
            ...
            async with sem:
                for attempt in range(3):
                    ...
                    try:
                        ...
                        if "invalid_api_key" in err or "incorrect api key" in err or "401" in err:
                            logger.error(
                                "_generate_embeddings: OPENAI KEY INVALID OR EXPIRED — "
                                "check OPENAI_API_KEY env var | error=%s",
                                e,
                            )
                            return None
...
async def _run_vectorize(
    ...
    replace: bool,
    use_ocr: bool = True,
) -> int:  # returns total chunks indexed (0 = nothing stored)
    ...
    if not replace:
        ...
        if already:
            logger.info(
                "_run_vectorize: %s already has docs in OpenSearch — "
                "proceeding with upsert (doc_as_upsert=True handles duplicates)",
                file_id,
            )
...
async def run_ocr_pipeline(
    ...
    try:
        ...
        _t = time.monotonic()
        chunks_indexed = await _run_vectorize(file_doc, local_path, replace)
        logger.info(
            "run_ocr_pipeline: vectorization done | chunks_indexed=%s | elapsed=%.3fs",
            chunks_indexed, time.monotonic() - _t,
        )

        if chunks_indexed == 0:
            _set_status("failed", "no_chunks_extracted")
            logger.error(
                "run_ocr_pipeline: file_id=%s produced 0 chunks — "
                "file may be empty, image-only, encrypted, or unsupported. "
                "Status set to 'failed'.",
                file_id,
            )
            return {"status": "failed", "file_id": file_id, "reason": "no_chunks_extracted"}
    ...
    try:
        for record in records:
            ...
            if file_size >= OCR_LARGE_FILE_THRESHOLD_BYTES:
                logger.error(
                    "handler: SKIPPED — file_id=%s size=%d bytes (%.2f MB) >= ECS threshold %d bytes; "
                    "this file should be routed to the ECS queue, not Lambda.",
                    file_id, file_size, file_size / (1024 * 1024), OCR_LARGE_FILE_THRESHOLD_BYTES,
                )
                continue
